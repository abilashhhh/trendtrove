...
const ProfilePageIndividualComponent: React.FC = () => {
  ...
  const handleOpenFollowersModal = () => setShowFollowersModal(true);
  const handleCloseFollowersModal = () => setShowFollowersModal(false);
  const handleOpenFollowingModal = () => setShowFollowingModal(true);
  ...
  useEffect(() => {
    if (userDetails) {
      ...
      const request = userDetails.requestedByMe?.find((r) => r.userId === currentUser._id);
      const pendingRequest = userDetails.requestsForMe?.find((r) => r.userId === currentUser._id);
      const mutualFollower = currentUser.followers.find((f) => f.userId === userDetails._id) &&
  ...
  if (!userDetails) {
    return   <main className="flex-1 pt-2 p-2 overflow-auto no-scrollbar bg-gray-800 dark:bg-gray-700 text-white p-10">
    <div className="mx-auto h-screen rounded-lg shadow-lg bg-slate-200 dark:bg-slate-800 overflow-auto p-5 no-scrollbar">User not found</div>
    </main>
  }
  return (
