    ...
    const handleDeleteAccount = async () => {
      Swal.fire({
        ...
        preConfirm: async (password) => {
          try {
            const deleteAccountResult = await deleteAccount(currentUser._id, password);
            if (deleteAccountResult.status === 'success') {
              return deleteAccountResult;
            } else {
              Swal.showValidationMessage(`Failed to delete account: ${deleteAccountResult.message}`);
            }
          } catch (error) {
  ...
  const navigate = useNavigate()
    const handleSuspendAccount = async () => {
      Swal.fire({
        ...
        preConfirm: async (password) => {
          try {
            const suspendAccountResult = await suspendAccount(currentUser._id, password);
            if (suspendAccountResult.status === 'success') {
              ...
              Swal.showValidationMessage(`Failed to suspend account: ${suspendAccountResult.message}`);
            }
          } catch (error) {
