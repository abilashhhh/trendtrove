    ...
    <>
      ...
      <main className="flex-1 min-h-screen pt-2 p-2 bg-gray-800 dark:bg-gray-700 text-black dark:text-white">
        <div className="p-4 rounded-lg bg-gray-100 dark:bg-gray-900 text-black dark:text-white h-full overflow-y-auto">
          <h1 className="text-2xl font-semibold underline text-center mb-4">Settings</h1>
  ...
  return (
    <div>
      ...
      <form className="space-y-4" onSubmit={handleChangePassword}>
        ...
        <button type="submit" className="bg-blue-500 dark:bg-blue-700 rounded p-2 hover:bg-blue-400 dark:hover:bg-blue-600">Update Password</button>
      </form>
    </div>
  );
};
...
const DeleteAccount = ({ currentUser }) => {
  ...
  return (
    <div>
      ...
      <button className="bg-red-500 dark:bg-red-700 rounded p-2 mt-4 hover:bg-red-400 dark:hover:bg-red-600" onClick={handleDeleteAccount}>
        Delete My Account
      </button>
    </div>
  );
...
// Suspend Account Component
const SuspendAccount = ({ currentUser }) => {
  const handleSuspendAccount = async () => {
    const userInfo = { _id: currentUser._id }; // Include necessary user info

    Swal.fire({
      title: 'Are you sure?',
      text: "Do you really want to suspend your account? You can reactivate it later.",
      icon: 'warning',
      showCancelButton: true,
